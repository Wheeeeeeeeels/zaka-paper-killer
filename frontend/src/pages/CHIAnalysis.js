...
function CHIAnalysis() {
  ...
  const fetchPapers = async () => {
    ...
    try {
      ...
      const response = await paperAPI.getPapers('chi');
  ...
  const handleSearch = async (value) => {
    ...
    try {
      ...
      const response = await paperAPI.searchPapers('chi', value);
  ...
  const handleTrackChange = async (value) => {
    ...
    try {
      ...
      const response = await paperAPI.getPapersByTrack('chi', value);
  ...
  const handleTopicChange = async (value) => {
    ...
    try {
      ...
      const response = await paperAPI.getPapersByTopic('chi', value);
      console.log('筛选结果:', response);
      setPapers(response);
    } catch (error) {
      console.error('筛选失败:', error);
  ...
  const handleWritePaper = () => {
    navigate('/write-paper?conference=chi');
  };
        ...
        <Col span={24}>
          <Card>
            <Space style={{ width: '100%', justifyContent: 'space-between' }}>
              <Title level={2}>CHI 2025 论文分析</Title>
              <Button 
                type="primary" 
                icon={<PlusOutlined />}
